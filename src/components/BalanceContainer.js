...
export default function BalanceContainer() {
    ...
    const [displayedBalances, setDisplayedBalances] = React.useState(JSON.parse(window.localStorage.getItem("displayedBalances")));
    ...
    useEffect(() => {
        ...
        } else { // balance has not been initialized yet
            reuseExistingBalance();
        }
    }, [])
    ...
    return (
        <div className={"balance-container"}>
            <div className={"balance"}>
                <img src={process.env.PUBLIC_URL + "/transferwise.png"} alt={'TransferWise'}
                ...
                <h2>
                    ...
                    <button className={'balance-container-button'} onClick={() => setModalOpen(!modalOpen)}>
                        <img src={process.env.PUBLIC_URL + "/settings.png"} alt={'settings'}
                             className={'settings-icon'}/>
                    </button>
                </h2>
                <Modal isOpen={modalOpen} shouldCloseOnOverlayClick={true} shouldCloseOnEsc={true} onRequestClose={() => setModalOpen(false)} style={{
                    overlay: {
                ...
                >
                    <SettingsModal balances={balance.balances} displayedBalances={displayedBalances}
